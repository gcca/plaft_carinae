...
exports._display =
  'Tipo de fondos con que se realizó la operación: consignar el código de acuerdo a la Tabla N° 4.'
  'Tipo de operación: consignar el código de acuerdo a la Tabla N° 5: Tipos de Operación'
  ...
  'Moneda en que se realizo la operacion: S=Nuevos Soles, USD =Dólares Americanos,E= Euros y O= Otra(Detallar en item siguiente)'
  ...
  'La operación ha sido calificada como sospechosa (1)Si, (2)No'
  'En caso en el item 41 haya consignado la opción (1) indicar el número de ROS con el que se remitió a la UIF.'
  'En caso en el item 41 haya consignado la opción (2) describir los argumentos por los cuales esta operación no fue calificada como sospechosa.'
...
exports._method =
  ...
  -> 'USD'
  -> 'No aplica'
  (d) -> (parseFloat d.'amount_soles').to-fixed 2
  (d) -> d.'exchange_rate'
  (d) -> if not d.'is_out' then d.'stakeholders'.'0'.'country' else 'No aplica'
  (d) -> if d.'is_out' then d.'stakeholders'.'0'.'country' else 'No aplica'
  (d) -> d.'description'
  (d) -> if d.'is_suspects' then 'Si' else 'No'
  (d) -> if d.'is_suspects' then d.'ros' else 'No aplica'
  (d) -> if not d.'is_suspects' then d.'suspects_by' else 'No aplica'
